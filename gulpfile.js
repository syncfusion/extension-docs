...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/extension-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/extension-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore' && changedFileName !== 'Jenkinsfile' && changedFileName !== 'README.md' && !changedFileName.includes('.gitlab')) {
                ...
                if (fs.existsSync('../extension-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/extension-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../extension-docs/` + changedFileName, `../../../gitlabRepo/extension-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/extension-docs/')) {
                            shelljs.cp('-rf', `../extension-docs/` + changedFileName, `../../../gitlabRepo/extension-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/extension-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/extension-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/extension-docs`);
